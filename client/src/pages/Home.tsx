...
export default function Home() {
  ...
  return <main className="academic-site">
    ...
    <section id="ecosistema" className="light-section section-anchor"><div className="section-wrap"><p className="kicker">04 · Ecosistema y transferencia</p><h2>Un aula que conversa con otros escenarios.</h2><p className="lead">La plataforma institucional sostiene el trayecto. Otros espacios lo expanden cuando responden a una propuesta, una consigna y una mediación docente.</p><div className="ecosystem-layers">{[{icon:<BookOpenCheck />, title:"Aula institucional", text:"Arquitectura del trayecto, materiales, actividades, acompañamiento y evaluación."},{icon:<MessageCircleMore />, title:"Comunicación expandida", text:"Canales que sostienen presencia, información y circulación de recursos."},{icon:<UsersRound />, title:"Producción colaborativa", text:"Espacios para crear, compartir, discutir y construir evidencias colectivas."}].map((item, index) => <motion.article key={item.title} initial={{ opacity: 1, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .35 }} transition={{ duration: .28, delay: index * .05 }}>{item.icon}<span>0{index + 1}</span><h3>{item.title}</h3><p>{item.text}</p></motion.article>)}</div><div className="ecosystem-note"><Network /><p><b>Cristóbal Cobo y Carina Lion:</b> la tecnología no es el centro del esquema; lo son las relaciones y decisiones que una propuesta habilita entre contenidos, personas, lenguajes y contextos.</p><button className="outline-action" onClick={() => setModal("ecosistema")}>Ver Telegram e Instagram <ArrowRight /></button></div><div className="transfer"><div><p className="kicker">Transferencia situada</p><h3>Diseñar respuestas para problemas reales de formación.</h3></div><p>Los Trabajos Finales Integradores articulan los saberes de la carrera con aulas de formación docente, propuestas de nivel superior, capacitaciones institucionales y comunidades de aprendizaje en equipos docentes.</p><ShieldCheck /></div></div></section>
